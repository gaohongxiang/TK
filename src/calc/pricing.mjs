import {
  PRICING_VIEWS,
  calcPricingRow,
  calcSalePrice as calcSalePriceFormula,
  derivePricingOrigPrice
} from './formulas.mjs';

function create({ state, els, helpers, shipping, save, document: rootDocument = globalThis.document }) {
  const {
    parseDiscounts,
    fmtCny,
    fmtDiscount,
  ...
  function calcRow(viewKey, origPrice, discount) {
    return calcPricingRow({
      state,
      totalCost: computeTotalCostNew(),
      viewKey,
      origPrice,
      discount
    });
  ...
  function syncInputsFromState() {
    ...
    if (rootDocument.activeElement !== els.shipCargoTypeNew) els.shipCargoTypeNew.value = state.shipCargoTypeNew;
    if (rootDocument.activeElement !== els.shipCargoTypeReview) els.shipCargoTypeReview.value = state.shipCargoTypeNew;
    ...
    if (els.shipCargoType && rootDocument.activeElement !== els.shipCargoType) els.shipCargoType.value = state.shipCargoType;
