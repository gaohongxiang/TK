...
function create({ state = {}, helpers = {}, ui = {} } = {}) {
  ...
  const productProviderFactory = helpers.productProviderFactory || ProductLibraryProviderFirestore;
  ...
  function getProductProvider() {
    ...
    if (!productProviderFactory?.create) return null;
    productProvider = productProviderFactory.create({
      state: productState,
      helpers: {
        nowIso
      }
