...
function normalizeOrderItems(items = [], { nowIso = () => new Date().toISOString() } = {}) {
...
function buildCourierSummary(items = [], field = 'company', options) {
...
function rawOrderNeedsCanonicalCleanup(data = {}, options) {
...
function getOrderCreatedAt(order, { nowIso = () => new Date().toISOString() } = {}) {
...
function normalizePulledOrder(data, options = {}) {
...
function buildOrderDoc(order, options = {}) {
...
function sortOrdersForSeqAssignment(left, right, options) {
  const leftCreatedAt = Date.parse(getOrderCreatedAt(left, options) || 0);
  const rightCreatedAt = Date.parse(getOrderCreatedAt(right, options) || 0);
