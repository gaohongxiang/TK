...
function normalizePulledOrder(raw: unknown, options: { nowIso?: () => string } = {}): OrderRecord {
  ...
  return {
    ...
    '结算金额': data?.settlementAmount == null ? '' : String(data.settlementAmount),
    '实际利润': data?.actualProfit == null ? '' : String(data.actualProfit),
...
function buildOrderDoc(order: OrderRecord, options: { nowIso?: () => string } = {}): OrderFirestoreDoc {
  ...
  return {
    ...
    settlementAmount: toNullableDecimal(order?.['结算金额']),
    actualProfit: toNullableDecimal(order?.['实际利润']),
