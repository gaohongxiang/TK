...
function computeOrderEstimatedProfit(order, exchangeRate) {
...
function normalizeOrderItem(item = {}, { nextUid = uid } = {}) {
  ...
  return {
    lineId: String(item?.lineId || '').trim() || nextUid(),
...
function buildOrderItemsSummary(items = [], options) {
  const groups = [];
...
function createOrderNormalizer({ constants = {}, nextUid = uid } = {}) {
  ...
  function normalizeOrderRecord(order) {
    ...
    delete next['入仓状态'];
    const items = normalizeOrderItems(next.items, itemOptions);
    if (items.length) {
      const totals = deriveOrderItemTotals(items, itemOptions);
      const summary = buildOrderItemsSummary(items, itemOptions);
      ...
      next['商品SKU ID'] = '';
      next['商品SKU名称'] = '';
      next['快递公司'] = buildOrderCourierSummary(items, 'company', itemOptions);
      next['快递单号'] = buildOrderCourierSummary(items, 'tracking', itemOptions);
    } else {
      delete next.items;
    }
    return next;
  ...
  function hasLegacyOrderStructure(order) {
    ...
    const hasItemCourier = items.some(item => {
      const normalized = normalizeOrderItem(item, itemOptions);
  ...
  return {
    ...
    normalizeOrderItem: item => normalizeOrderItem(item, itemOptions),
    normalizeOrderItems: items => normalizeOrderItems(items, itemOptions),
...
function hasLegacyOrderStructure(order, options) {
  return createOrderNormalizer(options).hasLegacyOrderStructure(order);
}

function cleanOrderToCurrentShape(order, options) {
  return createOrderNormalizer(options).cleanOrderToCurrentShape(order);
}

function migrateOrderToCurrentShape(order, options) {
...
function create({
  ...
  function cloneOrder(order) {
    return order ? normalizer.normalizeOrderRecord({ ...order }) : null;
  }
  ...
  function serializeOrder(order) {
    if (!order) return '';
    const normalized = normalizer.normalizeOrderRecord({ ...order });
    const sorted = {};
    Object.keys(normalized).sort().forEach(key => {
      const value = normalized[key];
      if (typeof value === 'undefined') return;
  ...
  function fromAccountSlot(slot) {
    return slot === safeConstants.UNASSIGNED_ACCOUNT_SLOT ? '' : slot;
  }

  function getAccountFileName(account) {
    const slot = toAccountSlot(account);
    if (slot === safeConstants.UNASSIGNED_ACCOUNT_SLOT) {
      return `${safeConstants.ACCOUNT_FILE_PREFIX}${safeConstants.UNASSIGNED_ACCOUNT_SLOT}${safeConstants.ACCOUNT_FILE_SUFFIX}`;
    }
    return `${safeConstants.ACCOUNT_FILE_PREFIX}${encodeURIComponent(slot)}${safeConstants.ACCOUNT_FILE_SUFFIX}`;
  }

  function parseAccountSlotFromFileName(filename) {
    if (!filename.startsWith(safeConstants.ACCOUNT_FILE_PREFIX) || !filename.endsWith(safeConstants.ACCOUNT_FILE_SUFFIX)) return null;
    const raw = filename.slice(safeConstants.ACCOUNT_FILE_PREFIX.length, -safeConstants.ACCOUNT_FILE_SUFFIX.length);
    if (!raw) return null;
    if (raw === safeConstants.UNASSIGNED_ACCOUNT_SLOT) return safeConstants.UNASSIGNED_ACCOUNT_SLOT;
    try {
  ...
  function detectScopedCourierCompany(trackingNumber) {
    return detectCourierCompany(trackingNumber, safeConstants.COURIER_AUTO_DETECTORS);
  }
  ...
  function maybeAutoDetectCourierFromForm({ force = false } = {}) {
    ...
    const detected = detectScopedCourierCompany(tracking.value);
  ...
  return {
    ...
    cleanOrderToCurrentShape: normalizer.cleanOrderToCurrentShape,
    migrateOrderToCurrentShape: normalizer.migrateOrderToCurrentShape,
    normalizeOrderRecord: normalizer.normalizeOrderRecord,
    normalizeOrderList,
    cloneOrder,
    normalizeOrderSeq,
    getOrderCreatedAt,
    ...
    detectCourierCompany: detectScopedCourierCompany,
    isOrderRefunded,
