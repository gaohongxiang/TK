...
function create({
  ...
  const escapeHtml = helpers.escapeHtml || (value => (
    TKHtml?.escape ? TKHtml.escape(value) : fallbackEscapeHtml(value)
  ));
  ...
  function promptProductExportAccounts() {
    return new Promise(resolve => {
      ...
      const modal = rootDocument?.querySelector?.('#pl-export-modal');
      const list = rootDocument?.querySelector?.('#pl-export-options');
      const allCheckbox = rootDocument?.querySelector?.('#pl-export-all');
      const cancelBtn = rootDocument?.querySelector?.('#pl-export-cancel');
      const confirmBtn = rootDocument?.querySelector?.('#pl-export-confirm');
