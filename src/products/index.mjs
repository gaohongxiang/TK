...
function createProductLibrary(options = {}) {
  ...
  const providerFactory = options.providerFactory ?? (
    typeof ProductLibraryProviderFirestore !== 'undefined' ? ProductLibraryProviderFirestore : rootWindow.ProductLibraryProviderFirestore
  );
  const tableView = options.tableView ?? (
    typeof ProductLibraryTableView !== 'undefined' ? ProductLibraryTableView : rootWindow.ProductLibraryTableView
  );
  const accountsFactory = options.accountsFactory ?? (
    typeof ProductLibraryAccounts !== 'undefined' ? ProductLibraryAccounts : rootWindow.ProductLibraryAccounts
  );
  const exportFactory = options.exportFactory ?? (
    typeof ProductLibraryExport !== 'undefined' ? ProductLibraryExport : rootWindow.ProductLibraryExport
  );
  const crudFactory = options.crudFactory ?? (
    typeof ProductLibraryCrud !== 'undefined' ? ProductLibraryCrud : rootWindow.ProductLibraryCrud
  );
  const html = options.html ?? (
    typeof TKHtml !== 'undefined' ? TKHtml : rootWindow.TKHtml
  );
  ...
  function formatFirestoreError(error, fallback = '商品管理操作失败') {
    ...
    if (code.includes('permission-denied') || /Missing or insufficient permissions/i.test(message)) {
      ...
      firestoreConnection?.notifyRulesUpdateNeeded?.(next);
      return next;
    }
  ...
  function getProviderMeta() {
    return registry?.getProvider?.('products', provider.key) || null;
  }
  ...
  async function copyLink(link) {
    ...
    try {
      if (firestoreConnection?.copyText) {
        await firestoreConnection.copyText(text);
      } else if (rootWindow.navigator?.clipboard?.writeText) {
  ...
  function getDisplayedProducts({ activeAccount = state.activeAccount } = {}) {
    return tableView?.deriveDisplayedProducts?.({
  ...
  const accountTools = accountsFactory.create({
    state,
  ...
  function renderMain() {
    renderAccountTabs();
    const pageState = tableView.render({
      toolbar: $('#pl-toolbar'),
...
let productLibraryInstance = null;

function getProductLibrary(options = {}) {
  if (productLibraryInstance && !options.forceNew) return productLibraryInstance;
  productLibraryInstance = createProductLibrary(options);
  return productLibraryInstance;
}

const ProductLibrary = {
  onEnter() {
    return getProductLibrary().onEnter();
  },
  formatFirestoreError(error, fallback) {
    return getProductLibrary().formatFirestoreError(error, fallback);
  }
};
