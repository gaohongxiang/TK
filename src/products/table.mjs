...
function clampPage(currentPage, pageSize, totalItems) {
  if (TKTableControls?.clampPage) return TKTableControls.clampPage(currentPage, pageSize, totalItems);
...
function buildTableToolbarMarkup({ pageSize, currentPage, totalPages, searchQuery, pageSizeOptions, includeSearch = false, disabled = false } = {}) {
  if (!TKTableControls?.buildTableToolbarMarkup) return '';
  return TKTableControls.buildTableToolbarMarkup({
    prefix: 'pl',
    pageSize,
    currentPage,
    totalPages,
...
function bindTableToolbar(container, { totalPages, includeSearch = false, onSearchChange, onPageSizeChange, onPageChange } = {}) {
  if (!TKTableControls?.bindTableToolbar) return;
  TKTableControls.bindTableToolbar(container, {
    prefix: 'pl',
    totalPages,
    includeSearch,
    onSearchChange,
