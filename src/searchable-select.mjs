...
function create(root, {
  ...
  requestAnimationFrame: raf = rootWindow?.requestAnimationFrame || (callback => setTimeout(callback, 0))
} = {}) {
  if (!root) return null;
  ...
  function positionPanel() {
    ...
    const viewportWidth = rootWindow?.innerWidth || rootDocument?.documentElement?.clientWidth || 0;
  ...
  function open() {
    ...
    rootWindow?.addEventListener?.('resize', handleFloatingUpdate);
    rootWindow?.addEventListener?.('scroll', handleFloatingUpdate, true);
    cleanupFloatingListeners = () => {
      rootWindow?.removeEventListener?.('resize', handleFloatingUpdate);
      rootWindow?.removeEventListener?.('scroll', handleFloatingUpdate, true);
    };
    if (searchInput) {
      searchInput.value = '';
      renderOptions();
      raf(() => searchInput.focus());
    }
  }
