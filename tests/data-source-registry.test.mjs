...
const srcOrderFirestoreSource = fs.readFileSync(path.join(root, 'src', 'orders', 'provider-firestore.ts'), 'utf8');
const srcProductFirestoreSource = fs.readFileSync(path.join(root, 'src', 'products', 'provider-firestore.ts'), 'utf8');
...
assert.match(
  reactOrdersSource,
  /import \{ OrderTrackerProviderFirestore \} from '\.\.\/\.\.\/\.\.\/orders\/provider-firestore\.ts'[\s\S]*OrderTrackerProviderFirestore\.create/,
...
assert.match(
  reactProductsSource,
  /import \{ ProductLibraryProviderFirestore \} from '\.\.\/\.\.\/\.\.\/products\/provider-firestore\.ts'[\s\S]*ProductLibraryProviderFirestore\.create/,
