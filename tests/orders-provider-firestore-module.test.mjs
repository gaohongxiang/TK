...
assert.match(
  esmSource,
  /__needsOrderCleanup:\s*rawOrderNeedsCanonicalCleanup\(data,\s*options\)/,
...
(async () => {
    ...
    const provider = providerModule.OrderTrackerProviderFirestore.create({
      state: {
        firestoreConfigText: '',
        firestoreProjectId: '',
        user: ''
    ...
    assert.deepEqual(
      ...
      'ESM Firestore provider 的配置解析结果应保持稳定'
    );
    ...
    assert.equal(pulledOrder['售价口径'], 'buyer_paid_shipping', 'ESM Firestore provider 拉取时应默认映射不包邮售价口径');
