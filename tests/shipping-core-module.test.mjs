...
const srcSource = fs.readFileSync(path.join(root, 'src', 'shipping-core.ts'), 'utf8');
...
assert.match(reactOrdersSource, /from '\.\.\/\.\.\/\.\.\/shipping-core\.ts'/, 'React 订单页需要显式导入共享运费核心');
assert.match(reactProductsSource, /from '\.\.\/\.\.\/\.\.\/shipping-core\.ts'/, 'React 商品页需要显式导入共享运费核心');
assert.match(reactCalculatorSource, /from '\.\.\/\.\.\/\.\.\/shipping-core\.ts'/, 'React 利润计算器需要显式导入共享运费核心');
assert.doesNotMatch(htmlSource, /<script src="js\/shipping-core\.js" defer><\/script>/, 'index.html 不应再加载旧共享运费核心普通脚本');

(async () => {
  const shippingModule = await import(`file://${path.join(root, 'src', 'shipping-core.ts')}`);
